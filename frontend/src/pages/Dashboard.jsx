...
import { Eye, Phone, MessageSquare, TrendingUp, CreditCard, Plus, Trash2, ToggleLeft, ToggleRight, BadgeCheck } from 'lucide-react';
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
      <div className="glass rounded-3xl p-6 w-full max-w-lg border border-white/10 max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <input value={form.title} onChange={e => setForm(p => ({ ...p, title: e.target.value }))} placeholder="Titlu anunț" required className="w-full bg-white/05 border border-white/10 rounded-xl px-4 py-2.5 text-sm text-white placeholder:text-white/25 focus:outline-none focus:border-brand-500/50" />
          <textarea value={form.description} onChange={e => setForm(p => ({ ...p, description: e.target.value }))} placeholder="Descriere serviciu..." rows={4} required className="w-full bg-white/05 border border-white/10 rounded-xl px-4 py-2.5 text-sm text-white placeholder:text-white/25 focus:outline-none focus:border-brand-500/50 resize-none" />
          <div className="grid grid-cols-2 gap-3">
            <input value={form.city} onChange={e => setForm(p => ({ ...p, city: e.target.value }))} placeholder="Oraș" className="bg-white/05 border border-white/10 rounded-xl px-4 py-2.5 text-sm text-white placeholder:text-white/25 focus:outline-none focus:border-brand-500/50" />
            <input value={form.region} onChange={e => setForm(p => ({ ...p, region: e.target.value }))} placeholder="Regiune" className="bg-white/05 border border-white/10 rounded-xl px-4 py-2.5 text-sm text-white placeholder:text-white/25 focus:outline-none focus:border-brand-500/50" />
          </div>
          <div>
            <p className="text-xs text-white/40 mb-2">Domenii</p>
            <div className="flex flex-wrap gap-2">
              {EXPERTISE_OPTIONS.map(e => (
                <button key={e} type="button" onClick={() => toggle('expertise', e)} className={`text-xs px-2.5 py-1 rounded-full border transition-colors ${form.expertise.includes(e) ? 'bg-brand-600/30 border-brand-500/50 text-brand-300' : 'bg-white/05 border-white/10 text-white/50'}`}>{e}</button>
              ))}
            </div>
          </div>
          ...
          <div className="flex gap-3">
            ...
            <button type="submit" disabled={loading} className="flex-1 py-2.5 rounded-xl bg-brand-600 hover:bg-brand-500 text-white font-semibold text-sm transition-colors disabled:opacity-50">{loading ? 'Se creează...' : 'Publică anunțul'}</button>
          </div>
        </form>
      </div>
    </div>
    ...
    <div className="min-h-screen bg-[#0f0f13]">
      ...
      {showNewListing && (
        <NewListingModal
          ...
          onCreated={() => { setShowNewListing(false); fetchDashboard(); }}
        />
      )}
      <main className="pt-24 pb-16 max-w-6xl mx-auto px-4">
        ...
        <div className="flex items-start justify-between mb-8 flex-wrap gap-4">
          <div>
            ...
            <p className="text-white/40 text-sm">Bun venit, {jurist?.firstName}!</p>
          </div>
          <div className="flex items-center gap-3">
            <SubBadge status={data?.subscription?.status} />
            {data?.subscription?.status === 'ACTIVE' ? (
              <button onClick={handlePortal} disabled={subLoading} className="flex items-center gap-2 px-4 py-2 rounded-xl bg-white/05 border border-white/10 text-sm text-white/70 hover:bg-white/10 transition-colors disabled:opacity-50">
                <CreditCard size={14} /> Gestionează abonament
              </button>
            ) : (
              <button onClick={handleSubscribe} disabled={subLoading} className="flex items-center gap-2 px-4 py-2 rounded-xl bg-brand-600 hover:bg-brand-500 text-sm text-white font-semibold transition-colors disabled:opacity-50">
        ...
        {subSuccess && (
          <div className="mb-6 p-4 rounded-2xl bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-sm flex items-center gap-2">
            <BadgeCheck size={16} /> Abonament activat cu succes! Puteți publica anunțuri acum.
          </div>
        )}
        ...
        {data?.subscription?.currentEnd && (
          <div className="mb-6 p-4 rounded-2xl glass border border-white/08 text-sm text-white/50">
            Abonament activ până la: <span className="text-white/80 font-medium">
              {new Date(data.subscription.currentEnd).toLocaleDateString('ro-RO')}
            </span>
          </div>
        )}
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          ...
          <StatCard icon={MessageSquare} label="Mesaje primite" value={data?.unreadMessages ?? 0} sub="necitite" color="blue" />
                ...
                <div key={l.id} className="glass rounded-2xl p-5 border border-white/08 flex items-center justify-between gap-4">
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-white/40 mt-0.5">
                      {l._count?.reviews ?? 0} recenzii · {new Date(l.createdAt).toLocaleDateString('ro-RO')}
                    </p>
                  </div>
