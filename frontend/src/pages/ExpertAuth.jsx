...
import { CATEGORIES, LANGUAGE_OPTIONS, MOLDOVA_REGIONS, PHONE_PREFIX, DEFAULT_COUNTRY } from '../lib/constants.js';

export default function ExpertAuth({ mode }) {
      ...
      <div className="relative w-full max-w-lg">
        ...
        <div className="glass rounded-3xl p-8 border border-white/08">
          ...
          <h1 className="text-2xl font-bold mb-1">
            {mode === 'login' ? 'Bun venit înapoi' : 'Creați cont de expert'}
          </h1>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {mode === 'register' && (
              <>
                ...
                <div className="pt-2">
                  <div className="h-px bg-white/06" />
                  <p className="text-xs text-white/30 mt-3 mb-2">Domeniu de activitate</p>
                  <select
                    value={form.category}
                ...
                {subProfessions.length > 0 && (
                  <div>
                    <p className="text-xs text-white/30 mb-2">Specializări</p>
                    <div className="flex flex-wrap gap-2">
                      {subProfessions.map((sp) => (
                        <button key={sp} type="button" onClick={() => toggleArr('areasOfExpertise', sp)}
                          className={`text-xs px-3 py-1.5 rounded-full border transition-colors ${form.areasOfExpertise.includes(sp) ? 'bg-brand-600/30 border-brand-500/50 text-brand-300' : 'bg-white/05 border-white/10 text-white/50 hover:border-white/20'}`}>
                          {sp}
                        </button>
                      ))}
                    </div>
          ...
          <p className="text-center text-sm text-white/40 mt-6">
            {mode === 'login'
              ? (<>Nu aveți cont? <Link to="/expert/inregistrare" className="text-brand-400 hover:text-brand-300">Înregistrați-vă</Link></>)
              : (<>Aveți deja cont? <Link to="/expert/login" className="text-brand-400 hover:text-brand-300">Autentificați-vă</Link></>)}
          </p>
        </div>
      </div>
    </div>
